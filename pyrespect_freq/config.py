"""
config.py — Configuration dataclass for pyReSpect-freq.
...
Supports direct instantiation or loading from a TOML or YAML file:
    ...
    # From TOML
    config = ReSpectConfig.from_toml("inp.toml")
...
# Internal mapping from FreqEnd strings to legacy integer codes
...
class ReSpectConfig:
    """
    Configuration for a pyReSpect-freq run.
    ...
    freq_end : {"lenient", "neutral", "strict"}
        Controls how far the s axis extends beyond the frequency window.
        - "lenient" : s extends to exp(±π/2) * 1/w  [legacy: 1]
        - "neutral" : s spans [1/w_max, 1/w_min]     [legacy: 2]
        - "strict"  : s contracts by exp(±π/2)        [legacy: 3]
        Default: "lenient".
    lam_min : float
    ...
    resample : bool
        Whether to resample 3-column input data onto a geometric grid.
        Has no effect on 5-column data (assumed pre-processed). Default: True.
    n_resample : int
...
def _flatten(d: dict) -> dict:
    """Flatten a one-level nested dict into a single dict.
